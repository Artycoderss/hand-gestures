  ...
  const FINGER_CONFIG = {
    index: { axes: { z: [80, 90, 70], y: [0, 0, 0], x: [0, 0, 0] } },
    middle: { axes: { z: [80, 90, 70], y: [0, 0, 0], x: [0, 0, 0] } },
    ring: { axes: { z: [80, 90, 70], y: [0, 0, 0], x: [0, 0, 0] } },
    pinky: { axes: { z: [80, 90, 70], y: [25, 0, 0], x: [0, 0, 0] } },
  };

  export function createThumbsUpTiltAnim() {
    ...
    const buildQuatXYZ = (xDeg, yDeg, zDeg) => {
      const qx = new THREE.Quaternion().setFromAxisAngle(axisVec.x, degToRad(xDeg));
      const qy = new THREE.Quaternion().setFromAxisAngle(axisVec.y, degToRad(yDeg));
      const qz = new THREE.Quaternion().setFromAxisAngle(axisVec.z, degToRad(zDeg));
        ...
        if (conf && conf.axes && bones.length) {
          for (let i = 0; i < bones.length; i += 1) {
            ...
            if (bq) {
              ...
              if (tx) {
                tq.multiply(
                  new THREE.Quaternion().setFromAxisAngle(axisVec.x, degToRad(tx * mul)),
                );
              }
              if (ty) {
                tq.multiply(
                  new THREE.Quaternion().setFromAxisAngle(axisVec.y, degToRad(ty * mul)),
                );
              }
              if (tz) {
                tq.multiply(
                  new THREE.Quaternion().setFromAxisAngle(axisVec.z, degToRad(tz * mul)),
                );
              }
    ...
    const isThumbsUp = (lm) => {
      ...
      const thumbOk = thumbExtendScore(lm, norm) >= THUMB_EXT_MIN;

      const curledIndex = isCurled(lm, INDEX_TIP, INDEX_PIP, norm);
      ...
      return thumbOk && curledIndex && curledMiddle && curledRing && curledPinky;
    };
    ...
    const update = ({ pivot, model, basePivotQuat, handVisible }) => {
      ...
      const aestheticQ = buildQuatXYZ(90, 0, 0);
