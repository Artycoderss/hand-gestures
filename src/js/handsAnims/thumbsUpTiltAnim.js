...
const HAND_SHIFT = new THREE.Vector3(1, -0.5, 0);
...
const FINGER_CONFIG = {
  thumb: { axes: { z: [0, 0, 0], y: [0, 0, 0], x: [0, 0, 0] } },
  index: { axes: { z: [90, 60, 50], y: [0, 0, 0], x: [0, 0, 0] } },
  middle: { axes: { z: [90, 60, 50], y: [0, 0, 0], x: [0, 0, 0] } },
  ring: { axes: { z: [90, 60, 50], y: [0, 0, 0], x: [0, 0, 0] } },
  pinky: { axes: { z: [90, 60, 30], y: [20, 0, 0], x: [0, 0, 0] } },
};
...
export function createThumbsUpTiltAnim() {
  ...
  const update = ({ pivot, model, basePivotQuat, handVisible }) => {
    ...
    const baseUpright = getUprightQuatFromBase(basePivotQuat);

    const deltaQ = buildQuatXYZ(ROT_X_DEG, ROT_Y_DEG, ROT_Z_DEG);
    const desired = deltaQ.clone().multiply(baseUpright);

    const finalQ = baseUpright.clone().slerp(desired, amt);
    ...
    if (amtRaw < 0.001) {
      model.position.copy(state.basePos);
    } else {
