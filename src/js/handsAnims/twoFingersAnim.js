...
export function createTwoFingersAnim() {
  ...
  const extScore = (lm, tip, pip, norm) => {
  ...
  const isExtended = (lm, tip, pip, norm) =>
    extScore(lm, tip, pip, norm) > 0.065;
  const isNotExtended = (lm, tip, pip, norm) =>
    extScore(lm, tip, pip, norm) < 0.04;
  ...
  const isTwoFingersUp = (lm) => {
    ...
    const middleUp = isExtended(lm, MIDDLE_TIP, MIDDLE_PIP, norm);

    const ringNotUp = isNotExtended(lm, RING_TIP, RING_PIP, norm);
    const pinkyNotUp = isNotExtended(lm, PINKY_TIP, PINKY_PIP, norm);

    return indexUp && middleUp && ringNotUp && pinkyNotUp;
  };
  ...
  const update = ({ pivot, model, basePivotQuat, handVisible }) => {
    ...
    const amt = smoothstep(clamp(state.strength, 0, 1));
